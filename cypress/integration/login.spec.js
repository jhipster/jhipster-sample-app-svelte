describe('User login', () => {
	...
	it('should require a valid username and password', () => {
		cy.getByTestId('loginForm').within(() => {
			...
			cy.getByName('password').type('invalid{enter}', { log: false })
		})
		cy.getByTestId('errorMsg').should('contain', 'Incorrect username or password.')
	})
	...
	it('should navigate to / on successful login', () => {
		cy.getByTestId('loginForm').within(() => {
			...
			cy.getByName('username').type('admin')
			cy.getByName('password').type('admin{enter}', { log: false })
		})
