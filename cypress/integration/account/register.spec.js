describe('Register User', () => {
	...
	it('should require mandatory fields', () => {
		cy.getByTestId('registerForm').contains('Create account').should('be.disabled')
	})
	...
	it('should not allow user account creation with duplicate username', () => {
		cy.getByTestId('registerForm').within(() => {
			...
			cy.getByName('email').type('joe@localhost.org')
			cy.getByName('password').type('jondoe', { log: false })
			cy.getByName('passwordConfirm').type('jondoe', { log: false })
			cy.root().contains('Create account').should('not.be.disabled').click()
		})
		cy.getByTestId('errorMsg').contains('Login name already in use. Please choose another one.')
	})

	it('should create new user account', () => {
		cy.getByTestId('registerForm').within(() => {
			cy.getByName('username').type('jon')
			cy.getByName('email').type('joe@localhost.org')
			cy.getByName('password').type('jondoe', { log: false })
			cy.getByName('passwordConfirm').type('jondoe', { log: false })
			cy.root().contains('Create account').should('not.be.disabled').click()
