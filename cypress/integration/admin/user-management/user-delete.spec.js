describe('User delete dialog page', () => {
	...
	beforeEach(() => {
		...
		cy.getByTestId('userMgmtTable')
			...
			.within($tr => {
				...
				cy.get('td').children().getByName('deleteBtn').click()
			})
	})
	...
	it('should display delete user dialog', () => {
		cy.getByTestId('svlModal').within(() => {
			cy.get('h1').should('be.visible').should('contain', 'Confirm delete operation')
			cy.get('p')
				.should('be.visible')
				.should('contain', 'Are you sure you want to delete the user?')
			cy.getByName('confirmDeleteBtn').should('not.be.disabled')
			cy.getByName('cancelBtn').should('not.be.disabled')
		})
	})
	...
	it('should delete the user', () => {
		cy.getByTestId('svlModal').within(() => cy.getByName('confirmDeleteBtn').click())

		cy.getByTestId('toast-success').contains('A user is deleted with identifier')
