...
class ExceptionTranslatorIT {
	...
	void testConcurrencyFailure() throws Exception {
		mockMvc
			...
			.andExpect(
				jsonPath("$.message")
					.value(ErrorConstants.ERR_CONCURRENCY_FAILURE)
			);
	}
	...
	void testMethodArgumentNotValid() throws Exception {
		mockMvc
			...
			.andExpect(
				jsonPath("$.message").value(ErrorConstants.ERR_VALIDATION)
			)
	...
	void testUnauthorized() throws Exception {
		mockMvc
			...
			.andExpect(
				jsonPath("$.path")
					.value("/api/exception-translator-test/unauthorized")
			)
			.andExpect(
				jsonPath("$.detail").value("test authentication failed!")
			);
	}
	...
	void testMethodNotSupported() throws Exception {
		mockMvc
			...
			.andExpect(
				jsonPath("$.detail")
					.value("Request method 'POST' not supported")
			);
	}
