...
class MailServiceIT {
	...
	private static final Pattern PATTERN_LOCALE_2 = Pattern.compile(
		"([a-z]{2})-([a-z]{2})"
	);
	...
	void testSendEmail() throws Exception {
		...
		assertThat(message.getAllRecipients()[0])
			.hasToString("john.doe@example.com");
		assertThat(message.getFrom()[0])
			.hasToString(jHipsterProperties.getMail().getFrom());
	...
	void testSendMultipartEmail() throws Exception {
		...
		assertThat(part.getDataHandler().getContentType())
			.isEqualTo("text/plain; charset=UTF-8");
	}
	...
	void testSendLocalizedEmailForAllSupportedLanguages() throws Exception {
		...
		for (String langKey : languages) {
			...
			String propertyFilePath =
				"i18n/messages_" + getJavaLocale(langKey) + ".properties";
			URL resource =
				this.getClass().getClassLoader().getResource(propertyFilePath);
	...
	private String getJavaLocale(String langKey) {
		...
		if (matcher2.matches()) {
			javaLangKey =
				matcher2.group(1) + "_" + matcher2.group(2).toUpperCase();
		}
