...
public class HibernateTimeZoneIT {
	...
	public void setup() {
		...
		dateTimeWrapper.setLocalDateTime(
			LocalDateTime.parse("2014-11-12T07:50:00.0")
		);
		dateTimeWrapper.setOffsetDateTime(
			OffsetDateTime.parse("2011-12-14T08:30:00.0Z")
		);
		dateTimeWrapper.setZonedDateTime(
			ZonedDateTime.parse("2011-12-14T08:30:00.0Z")
		);
		...
		timeFormatter =
			DateTimeFormatter.ofPattern("HH:mm:ss").withZone(ZoneId.of(zoneId));
	...
	public void storeInstantWithZoneIdConfigShouldBeStoredOnGMTTimeZone() {
		...
		String expectedValue = dateTimeFormatter.format(
			dateTimeWrapper.getInstant()
		);
	...
	public void storeLocalDateTimeWithZoneIdConfigShouldBeStoredOnGMTTimeZone() {
		...
		String request = generateSqlRequest(
			"local_date_time",
	...
	private String generateSqlRequest(String fieldName, long id) {
		return format(
			"SELECT %s FROM jhi_date_time_wrapper where id=%d",
			fieldName,
			id
		);
	}
