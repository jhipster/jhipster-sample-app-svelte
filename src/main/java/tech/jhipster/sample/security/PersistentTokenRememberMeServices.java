...
public class PersistentTokenRememberMeServices
	...
	public PersistentTokenRememberMeServices(
		...
		upgradedTokenCache =
			new PersistentTokenCache<>(UPGRADED_TOKEN_VALIDITY_MILLIS);
	}
	...
	protected UserDetails processAutoLoginCookie(
		...
		synchronized (this) { // prevent 2 authentication requests from the same user in parallel
			String login = null;
			UpgradedRememberMeToken upgradedToken = upgradedTokenCache.get(
				cookieTokens[0]
			);
			if (upgradedToken != null) {
				login = upgradedToken.getUserLoginIfValid(cookieTokens);
				log.debug(
					"Detected previously upgraded login token for user '{}'",
					login
				);
			}
	...
	public void logout(
		...
		if (rememberMeCookie != null && rememberMeCookie.length() != 0) {
			try {
				...
				log.info(
					"Invalid cookie, no persistent token could be deleted",
					ice
				);
			} catch (RememberMeAuthenticationException rmae) {
				log.debug(
					"No persistent token found, so no token could be deleted",
					rmae
				);
			}
		}
	...
	private PersistentToken getPersistentToken(String[] cookieTokens) {
		...
		Optional<PersistentToken> optionalToken = persistentTokenRepository.findById(
			presentedSeries
		);
	...
	private static class UpgradedRememberMeToken implements Serializable {
		...
		String getUserLoginIfValid(String[] currentToken) {
			if (
				currentToken[0].equals(this.upgradedToken[0]) &&
